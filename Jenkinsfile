...
pipeline {
    ...
    environment {
		...
		GIT_REPOSITORY = 'tailoringexpert/db-migration.git'	
    ...
    stages {
        stage('checkout') {
             steps {
                script {
                    ...
                    currentBuild.displayName = "#${env.BUILD_ID} | ${checkoutBranch}"
                }
        ...
        stage('build') {
            steps {
				sh "mvn --settings .jenkins/settings.xml -DskipTests clean compile"
            }
        }

        stage('verify') {
            steps {
				sh "mvn --settings .jenkins/settings.xml verify"
            }
		...
		stage("quality gate") {
            steps {
			    withSonarQubeEnv('default') {
	            	sh "mvn --settings .jenkins/settings.xml sonar:sonar"
                }
